...
for (const path of ["app", "public", "migrations", "docs", "README.md", ".env.example", "launcher.mjs", "START-SIGNAL-EXPERT.cmd", "start-signal-expert.sh", "START-SIGNAL-EXPERT-4020.cmd", "start-signal-expert-4020.sh"]) {
...
writeFileSync(resolve(packageRoot, "package.json"), `${JSON.stringify({
  ...
  type: "module",
  engines: { node: ">=22.5" },
  scripts: { start: "node launcher.mjs", "start:4020": "node launcher.mjs --instance phase2 --port 4020 --strict-port --database data/instances/phase2/signal-expert.db --stream", server: "node app/server.mjs", "db:migrate": "node scripts/migrate.mjs" },
}, null, 2)}\n`);
