...
Version 0.6.1 fixes local HTTP 429 errors caused by dashboard polling. Each three-second refresh now reads one aggregate local endpoint instead of five separate endpoints, overlapping refreshes are coalesced, and hidden tabs pause polling until visible again. Normal traffic drops from 100 to 20 local requests per minute per visible tab while the existing 180-request safety limit remains active.

The v0.6 signal workflow keeps the durable manual lifecycle and applies one auditable gate shared by manual signals and PAPER execution. It does **not** place an order. `READY` means the Spot-proxy research checks passed; it is not an exchange acknowledgement, a verified Event Futures quote, or evidence that an order was accepted. A candidate can become `READY` only when a completed directional 1m pattern is still inside its timestamped grace period, completed 5m structure and completed 15m trend confirm the same direction, quality and finite invalidation pass, ticker/candles are fresh, and the attributed Spot top of book passes spread, liquidity, provider, and timestamp-coherence limits. Candles, ticker, and order book must come from the same provider before a setup is actionable; independent provider bundles may remain visible for research but fail the entry gate when their sources differ. If the optional macro filter is enabled, its attributed calendar must also be fresh and outside a high-impact USD blackout window. If it is disabled, the audit status is `SKIPPED · DISABLED`: no macro/news filtering occurred, and the result is not `CLEAR` or `PASS`.
