...
export const config = Object.freeze({
  ...
  depthSnapshotLimit: numberValue("DEPTH_SNAPSHOT_LIMIT", 5000, { min: 100, max: 5000, integer: true }),
  depthTopLevelLimit: numberValue("DEPTH_TOP_LEVEL_LIMIT", 20, { min: 20, max: 100, integer: true }),
  depthMaxBufferEvents: numberValue("DEPTH_MAX_BUFFER_EVENTS", 10000, { min: 100, max: 100000, integer: true }),
  depthMaxLevels: numberValue("DEPTH_MAX_LEVELS", 5000, { min: 100, max: 20000, integer: true }),
  orderFlowStaleAfterMs: numberValue("ORDER_FLOW_STALE_AFTER_MS", 15000, { min: 1000, max: 300000, integer: true }),
  orderFlowTradeStaleAfterMs: numberValue("ORDER_FLOW_TRADE_STALE_AFTER_MS", 15000, { min: 1000, max: 300000, integer: true }),
  orderFlowRetentionMs: numberValue("ORDER_FLOW_RETENTION_MS", 300000, { min: 300000, max: 3600000, integer: true }),
  orderFlowLiquidityWindowMs: numberValue("ORDER_FLOW_LIQUIDITY_WINDOW_MS", 30000, { min: 1000, max: 300000, integer: true }),
  orderFlowSpoofLifetimeMs: numberValue("ORDER_FLOW_SPOOF_LIFETIME_MS", 30000, { min: 1000, max: 300000, integer: true }),
  orderFlowMaxTrades: numberValue("ORDER_FLOW_MAX_TRADES", 100000, { min: 1000, max: 1000000, integer: true }),
  alertCooldownMs: numberValue("ALERT_COOLDOWN_MS", 60000, { min: 0, max: 86400000, integer: true }),
  alertHysteresisMs: numberValue("ALERT_HYSTERESIS_MS", 0, { min: 0, max: 300000, integer: true }),
  operationalWindowSize: numberValue("OPERATIONAL_WINDOW_SIZE", 256, { min: 16, max: 10000, integer: true }),
  operationalStallAfterMs: numberValue("OPERATIONAL_STALL_AFTER_MS", 120000, { min: 5000, max: 3600000, integer: true }),
  ...
  calibrationBinCount: numberValue("CALIBRATION_BIN_COUNT", 10, { min: 2, max: 50, integer: true }),
  calibrationThreshold: numberValue("CALIBRATION_THRESHOLD", 0.5, { min: 0.01, max: 0.99 }),
  replayRunsLimit: numberValue("REPLAY_RUNS_LIMIT", 50, { min: 1, max: 500, integer: true }),
