...
export function deriveOrderBookMetrics(orderBook) {
  ...
  return {
    ...
    topNotional: Math.min(bestBidLevel.price * bestBidLevel.quantity, bestAskLevel.price * bestAskLevel.quantity),
    midPrice,
  };
}
...
export function evaluateEntryGates({ snapshot, candidate, policy, now = new Date() }) {
  ...
  add("TOP_LIQUIDITY", metrics.valid === true && metrics.topNotional >= policy.minTopNotional, metrics.valid === true ? `Minimum top-side notional ${metrics.topNotional.toFixed(2)} USDT; required ${policy.minTopNotional.toFixed(2)}.` : "Top-of-book liquidity cannot be calculated.", { topNotional: metrics.topNotional, minimum: policy.minTopNotional });
