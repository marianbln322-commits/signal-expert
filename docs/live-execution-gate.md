...
The v0.6 manual signal desk may show `SETUP PROXY VALID ↑ UP` or `SETUP PROXY VALID ↓ DOWN` only inside its timestamped research window. `↑ UP` means the Spot-proxy price is expected to finish above the recorded proxy observation; `↓ DOWN` means below it. This state is not a live-entry instruction or order acknowledgement: the application has no verified Event Futures contract, quote, payout, liquidity, settlement feed, credential, account, signing, or placement method. Any operator considering a real action must independently verify every one of those product facts on MEXC. The recorded observation and horizon outcome are attributed Spot-proxy values classified `NOT_EVENT_FUTURES_SETTLEMENT`. Setup quality remains a deterministic rules score, not probability. A measured confidence percentage remains unavailable until the prospective symbol+horizon sample reaches its configured minimum, then includes a Wilson 95% interval and the explicit classification `SPOT_PROXY_PROSPECTIVE_OUTCOMES_NOT_EVENT_FUTURES_CALIBRATION`.

The v0.6 completed-candle strategy uses 1h context, 15m/5m structure, an objective completed 1m trigger, explicit completed 5m confirmation and completed 15m alignment to rank separate 10m/30m setups. Forming candles are excluded and deterministic close-time keys prevent duplicate signals. EMA20/50, FVG/IFVG, liquidity sweep, CHoCH/MSS, horizon-specific support/resistance and explicit finite invalidation components are objective and auditable. Before either manual readiness or a local PAPER open, the same policy checks trigger freshness, Spot spread, cumulative near-book liquidity, one provider across candles/ticker/book, receipt-time coherence, and macro/news state. A disabled macro filter is recorded as `SKIPPED` (not filtered), never `CLEAR`; enabled stale/unavailable/blackout states block. READY research records receive a live non-mutating gate recheck; changed conditions block the setup without changing its immutable creation audit. Transiently blocked manual/autonomous candidates may be reevaluated only while the original trigger remains current. A statistically UNDERPERFORMING manual segment can be held at WAIT, but no algorithm can guarantee future accuracy or substitute Spot outcomes for the product's authoritative settlement.

The autonomous v0.6 workflow remains a local PAPER shadow experiment. The complete shared entry policy is rerun immediately before persistence to stop stale or changed conditions. A previous loss does not create a new signal, no simulation profile may bypass risk limits, and the configured daily stop-at-profit is a safety pause rather than promised income.
