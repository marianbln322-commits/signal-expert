...
The completed-candle model uses 1h context, 15m/5m structure and a 1m trigger to rank separate 10m/30m PAPER setups. Forming candles are excluded and deterministic close-time keys prevent duplicate autonomous entries. EMA20/50, FVG/IFVG, liquidity sweep, CHoCH/MSS and explicit finite invalidation components are objective and auditable. Setup quality controls whether a paper trade is eligible and may select a bounded 1×/1.5×/2× stake tier, but it is not represented as calibrated probability. Prospective symbol+horizon safeguards use Wilson 95% bounds against break-even rather than raw small samples. A previous loss does not create a new signal, and neither the `ADAPTIVE_CAPPED` nor exact `OBSERVED_10_30_90_270` simulation profile may bypass setup, cash, 2% bankroll, absolute, daily loss, one-position, or correlated-exposure limits. The configured daily stop-at-profit is only a safety pause after measured PAPER profit, not promised income. The v0.3.0 autonomous service opens only local SQLite paper positions and contains no MEXC credential, signing, or order-placement operation.
